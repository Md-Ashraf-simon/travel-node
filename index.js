...
const uri =
  "mongodb+srv://firstdb:dCKyksU6QdosDVcf@cluster0.vwcea.mongodb.net/myFirstDatabase?retryWrites=true&w=majority";
...
async function run() {
  try {
    ...
    app.get("/ordersData/:id", async (req, res) => {
      const id = req.params.id;
      const query = { _id: ObjectId(id) };
      const singleTour = await orderCollection.findOne(query);
      // console.log('load user with id: ', id);
      res.send(singleTour);
    });
